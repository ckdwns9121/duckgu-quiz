...
8. **출력값 정답은 손으로 쓰지 않는다.** 코드를 실제로 실행한 결과를 쓴다. 프론트엔드는 `tools/frontend/cases.mjs`에 코드를 두고 `node tools/frontend/run.mjs`로 `answers.json`을 만든다. DOM 동작은 jsdom으로 확인한다.
...
4. 폰 화면을 확인한다. 긴 코드 줄은 폰에서 가로 스크롤이 생기므로 한 줄에 한 동작으로 나눈다(대략 40자 안쪽).
