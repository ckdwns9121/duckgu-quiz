...
from fe_quiz_render_perf import UNIT_RENDER, UNIT_RENDER_ORDER, UNIT_PERF, UNIT_PERF_ORDER
...
# 4차: 렌더링 방식은 React 뒤·Next.js 앞, 성능 최적화는 Next.js 뒤
NEW = {**NEW, UNIT_RENDER[0]: UNIT_RENDER[3], UNIT_PERF[0]: UNIT_PERF[3]}
ORDER = {**ORDER, UNIT_RENDER[0]: UNIT_RENDER_ORDER, UNIT_PERF[0]: UNIT_PERF_ORDER}
_ids = [u[0] for u in BASE]
BASE.insert(_ids.index('fe-next'), (UNIT_RENDER[0], UNIT_RENDER[1], UNIT_RENDER[2], []))
_ids = [u[0] for u in BASE]
BASE.insert(_ids.index('fe-next') + 1, (UNIT_PERF[0], UNIT_PERF[1], UNIT_PERF[2], []))
...
write_notes(
    ...
    lede='출근길 IT 퀴즈의 프론트엔드 코스 노트입니다. JavaScript, 비동기, 브라우저, CSS, React, 렌더링 방식, Next.js, 성능 최적화, 웹 기본기를 면접과 실무에서 자주 헷갈리는 것 위주로 모았습니다. <b>퀴즈 모드</b>를 켜면 답이 가려집니다.',
